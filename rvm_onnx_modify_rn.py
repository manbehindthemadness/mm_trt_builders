...
def modify(input: str, output: str, downsample_ratio: float) -> None:
    print(f'\nonnx load: {input}')
    ...
    scales = gs.Constant('398',
        np.asarray([1, 1, downsample_ratio, downsample_ratio], dtype=np.float32))

    resize_3.inputs = [i if i.name != '398' else scales for i in resize_3.inputs]
    print()
    print(resize_3)
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('-i', '--input', type=str, default='rvm_mobilenetv3_fp32_sim.onnx')
    parser.add_argument('-o', '--output', type=str,)
    parser.add_argument('--downsample-ratio', type=float)
    parser.add_argument('--input-size', type=int, default=[2560, 1440], nargs=2,
    ...
    if args.input_size is not None:
        w, h = args.input_size
        if args.downsample_ratio is None:
...
#
# trtexec --onnx=rvm_mobilenetv3_fp32_sim_modified.onnx --fp16 --workspace=64 --saveEngine=rvm_mobilenetv3_fp32_ds025.engine
